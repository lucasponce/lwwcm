...
%>
    ...
    <li data-id="<%= u.getId() %>" data-mimetype="<%= (u.getMimeType() != null ? u.getMimeType():"") %>" data-filename="<%= u.getFileName() %>">
        ...
        <a href="#" id="${n}upload<%= u.getId() %>" <% if (u.getMimeType() != null && u.getMimeType().startsWith("image")) { %>onmouseover="showPreview('<%= n %>', this.id, '/lwwcm/rs/u/<%= u.getId() %>');" onmouseout="hidePreview('<%= n %>', this.id, '${downloadUploadEvent}');" <% } %>><span class="lwwcm-upload-mimetype"><%= u.getMimeType() %></span></a> <% if (u.getMimeType() != null && u.getMimeType().startsWith("image")) { %><span class="lwwcm-upload-mimetype glyphicon glyphicon-eye-open margin-top"></span><% } %>
    </li>
<%
        }
    }
