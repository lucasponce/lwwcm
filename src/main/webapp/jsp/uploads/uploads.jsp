            ...
            <td>
                <div>
                    ...
                    <div class="lwwcm-post-categories"><%
                        ...
                        %>[<a href="javascript:showFilterCategoriesById('${n}', '<%= cU.getId() %>');"><%= cU.getName() %></a><% if (canWrite) { %> <a href="${removeCategoryUploadAction}&catid=<%= cU.getId() %>&uploadid=<%= u.getId() %>" class="lwwcm-delete-category"><span class="glyphicon glyphicon-remove middle"></span></a><% } %>] <%
                                }
                            }
                        %>
                    </div>
                    <% if (canWrite) { %>
                    <div class="lwwcm-post-actions"><a href="${editUploadView}&editid=<%= u.getId() %>">Edit</a> | <a href="javascript:deleteUpload('${n}', <%= u.getId() %>)">Delete</a> | <a href="javascript:;" onclick="javascript:showSingleCategories('${n}', this.id, '<%= u.getId() %>');" id="${n}addCategory<%= u.getId() %>" >Category</a> | <a href="javascript:;" onclick="javascript:showSingleAclUpload('${n}', this.id, '${showUploadAclsEvent}', '<%= u.getId() %>', '${uploadsView}');" id="${n}addAcl<%= u.getId() %>">Security</a></div>
                    <% } %>
                </div>
            </td>
